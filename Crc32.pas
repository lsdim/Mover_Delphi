...
function StrCRC(const STR: string): cardinal;
function FileCRC32(const FileName: string): Cardinal;
function StreamCRC32(const InFile: tStream): Cardinal;
...
begin
  ...
  crc32 := $FFFFFFFF;
  try
    InFile := TStringStream.Create(STR);
    ...
    begin
      if Assigned(InFile) then
        InFile.Free;
      ShowMessage(Format('При обробці файлу [%s] вийшла ' +
        'ось така помилка [%s]', [STR, E.Message]));
    end;
  end;
  Result := not crc32;
end;
  ...
  try
    ...
    begin
      if Assigned(InFile) then
        InFile.Free;
      ShowMessage(Format('При обробці файлу [%s] вийшла ' +
        'ось така помилка [%s]', [FileName, E.Message]));
    end;
  end;
  Result := not crc32;
end;
...
begin
  ...
  crc32 := $FFFFFFFF;
  try
//    InFile := TStream.Create;
//    inFile.CopyFrom(FS,FS.Size);
//    showmessage(inttostr(inFile.Size));
    Res := InFile.Read(Buf, BufLen);
    while (Res <> 0) do
    begin
      crc32 := UpdateCrc32(crc32, BufPtr, Res);
...
//    InFile.Destroy;
    ...
    begin
     // if Assigned(InFile) then
 //       InFile.Free;
      ShowMessage(Format('При обробці вийшла' +
        'ось така помилка [%s]', [E.Message]));
    end;
  end;
  Result := not crc32;
end;
